...
export default function LeadDetailPage() {
  ...
  if (loading && !lead) {
    return (
      <div className="flex justify-center items-center min-h-[500px] text-text-muted font-sans text-sm">
  ...
  if (error || !lead) {
    return (
      <div className="max-w-md mx-auto py-24 text-center space-y-4 font-sans text-sm px-6">
        ...
        <h4 className="font-bold text-text-primary text-lg">Lead no encontrado</h4>
        <p className="text-text-muted">
          {error || "El identificador de oportunidad ingresado no existe."}
        </p>
        <Link 
          href="/crm"
          className="inline-flex items-center gap-2 px-4 py-2 mt-4 bg-accent-cyan text-white rounded-md hover:bg-accent-cyan/90 transition-colors"
        >
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-8 space-y-6 bg-bg-secondary min-h-screen font-sans text-text-primary">
      
      {/* Header Back Button */}
      <Link 
        href="/crm"
        className="inline-flex items-center gap-1 text-text-muted hover:text-blue-600 text-sm font-medium transition-colors"
      >
        <ArrowLeft className="w-4 h-4" /> Volver al Pipeline
      </Link>

      {/* Title & Stage Selector Area */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-bg-primary p-6 rounded-lg border border-border-subtle shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border tracking-wide ${
              ...
              lead.riskLevel === "SPAM" ? "bg-bg-tertiary text-text-muted border-border-medium" :
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-semibold text-text-secondary">Etapa:</span>
          <select
            value={lead.status}
            disabled={statusUpdating}
            onChange={(e) => handleStatusChange(e.target.value)}
            className="bg-bg-primary border border-border-medium rounded-md py-2 px-4 text-sm text-text-primary font-semibold focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider border-b border-border-subtle pb-2 flex items-center gap-2">
              <Building2 className="w-4 h-4 text-text-muted" /> Información General
            </h3>
            <div className="space-y-3 text-sm">
              <div className="flex flex-col">
                <span className="text-text-muted text-xs">Contacto</span>
                <span className="font-medium text-text-primary">{lead.fullName}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-text-muted text-xs">Correo Electrónico</span>
              ...
              <div className="flex flex-col">
                ...
                <a href={`tel:${lead.phone}`} className="font-medium text-text-primary hover:text-blue-600 flex items-center gap-1.5">
              ...
              <div className="flex flex-col">
                <span className="text-text-muted text-xs">Origen de Captura</span>
                <span className="font-medium text-text-primary capitalize">{lead.source}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle shadow-sm space-y-4">
            <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider border-b border-border-subtle pb-2 flex items-center gap-2">
              <Briefcase className="w-4 h-4 text-text-muted" /> Datos Comerciales
            </h3>
            <div className="space-y-4 text-sm">
              <div className="bg-bg-secondary p-3 rounded-md border border-border-subtle">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-text-muted text-xs">Valor Estimado:</span>
                  <span className="font-bold text-text-primary">${valEstimado.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center mb-1">
                  <span className="text-text-muted text-xs">Probabilidad de Cierre:</span>
                  <span className="font-bold text-blue-600">{prob}%</span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t border-border-subtle mt-2">
                  <span className="text-text-muted text-xs font-semibold">Valor Ponderado:</span>
                  <span className="font-bold text-emerald-600">${valPonderado.toLocaleString()}</span>
                </div>
              </div>
              
              <div className="flex flex-col gap-1">
                <span className="text-text-muted text-xs">Responsable Asignado</span>
                <select
                  value={assignedTo}
                  onChange={(e) => setAssignedTo(e.target.value)}
                  onBlur={handleSaveCommercialData}
                  disabled={isSavingCommercial}
                  className="bg-bg-secondary border border-border-subtle rounded-md px-2 py-1.5 text-sm font-medium text-text-primary outline-none focus:ring-1 focus:ring-blue-500"
                >
              ...
              <div className="flex flex-col gap-1 pt-2 border-t border-border-subtle mt-2">
              ...
              <div className="flex flex-col gap-1 pt-2 border-t border-border-subtle mt-2">
                ...
                <input 
                  ...
                  onBlur={handleSaveCommercialData}
                  disabled={isSavingCommercial}
                  className="bg-bg-secondary border border-border-subtle rounded-md px-2 py-1.5 text-xs text-text-primary outline-none focus:ring-1 focus:ring-blue-500"
                />
              </div>

              <div className="flex flex-col gap-1 mt-1">
                <span className="text-text-muted text-xs">Próxima Tarea</span>
                <input 
                  type="text" 
          ...
          {diagnostic && (
            <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle shadow-sm space-y-4">
              <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider border-b border-border-subtle pb-2 flex items-center gap-2">
                <Target className="w-4 h-4 text-text-muted" /> Necesidad Técnica
              </h3>
              <div className="space-y-3 text-sm">
                <div className="flex flex-col">
                  <span className="text-text-muted text-xs">Servicio Solicitado</span>
                  <span className="font-medium text-text-primary capitalize">{lead.serviceType}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-text-muted text-xs">Entorno Operativo</span>
                  <span className="font-medium text-text-primary capitalize">{lead.environmentType.replace("_", " ")}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-text-muted text-xs">Caudal Referencial Estimado</span>
                  <span className="font-medium text-text-primary">{diagnostic.airflow ? `${diagnostic.airflow.toLocaleString()} m³/h` : "No calculado"}</span>
                </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Note Input Area */}
          <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle shadow-sm">
            ...
            <form onSubmit={handleAddNote} className="space-y-4">
              <div className="flex gap-4">
                <div className="w-1/3">
                  <label className="block text-xs font-semibold text-text-muted mb-1">Tipo de Actividad</label>
                  <select
                    value={noteType}
                    onChange={(e) => setNoteType(e.target.value)}
                    className="w-full border border-border-medium rounded-md py-2 px-3 text-sm text-text-secondary focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none"
                  >
                ...
                <div className="w-2/3">
                  <label className="block text-xs font-semibold text-text-muted mb-1">Descripción</label>
                  <textarea
                    rows={2}
                    value={newNote}
                    onChange={(e) => setNewNote(e.target.value)}
                    placeholder="Escriba los detalles del seguimiento, acuerdos o próxima acción..."
                    className="w-full border border-border-medium rounded-md py-2 px-3 text-sm text-text-secondary focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none resize-none"
                  />
                </div>
              </div>
              <div className="flex justify-end">
                <button
                  ...
                  className="px-6 py-2 bg-accent-cyan hover:bg-accent-cyan/90 text-white text-sm font-semibold rounded-md transition-colors flex items-center gap-2 disabled:opacity-50"
                >
          ...
          <div className="bg-bg-primary p-5 rounded-lg border border-border-subtle shadow-sm">
            <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider mb-6 flex items-center gap-2">
              <Activity className="w-4 h-4 text-text-muted" /> Historial de Actividades
            </h3>
            
            <div className="relative pl-6 border-l-2 border-border-subtle space-y-8 pb-4">
              {logs.map((log) => {
                let iconBg = "bg-bg-tertiary text-text-muted border-border-subtle";
                ...
                return (
                  <div key={log.id} className="relative">
                    <div className={`absolute -left-[35px] top-0.5 h-6 w-6 rounded-full border flex items-center justify-center bg-bg-primary ${iconBg.split(" ")[2]}`}>
